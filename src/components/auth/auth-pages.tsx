...
function LoginForm({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const res = await api.post<SafeUser>("/api/auth/login", {
        email,
        password,
      });
      onSuccess(res.data!);
      await swal.success(
        "Login Berhasil!",
        `Selamat datang, ${res.data!.namaLengkap}`,
      );
    } catch (e) {
    ...
    >
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <Button
          type="submit"
          disabled={loading}
          className="w-full h-11 gradient-primary text-white border-0 hover:opacity-90"
        >
...
function RegisterForm({
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    const err: Record<string, string> = {};
    if (!form.namaLengkap || form.namaLengkap.length < 3)
      err.namaLengkap = "Nama minimal 3 karakter";
    ...
    if (form.password !== form.konfirmasiPassword)
      err.konfirmasiPassword = "Konfirmasi password tidak cocok";
    if (!form.nomorKartu || form.nomorKartu.length < 4)
      err.nomorKartu = "Nomor kartu min. 4 karakter";
    setErrors(err);
    if (Object.keys(err).length) return;
    ...
    try {
      ...
      await swal.success(
        "Registrasi Berhasil!",
        `Akun ${res.data!.namaLengkap} dibuat. Anda akan diarahkan ke dashboard.`,
      );
      onSuccess(res.data!);
    } catch (e) {
      const msg = e instanceof ApiError ? e.message : "Terjadi kesalahan";
  ...
  return (
    ...
    >
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <Field
          label="Nomor Kartu Karyawan"
          icon={<IdCard className="h-4 w-4" />}
          error={errors.nomorKartu}
        >
          <Input
            value={form.nomorKartu}
            onChange={(e) => set("nomorKartu", e.target.value)}
            placeholder="UID RFID / NFC / QR Code"
            className="pl-10"
          />
        </Field>
